...
router.get('/getRace', function (req, res) {
    sql.connect(dbConfig.dbConnection()).then(() => {
        return sql.query("select [PERSON_ID], [RACE], Category = 'Risk Factors - Non Red Flag' from [dbo].[VISITS] where [RACE] like '%African American%'");
    }).then(result => {
        console.log(" query result  ", result);
        res.json({ data: result.recordset })
    }).catch(err => {
...
router.get('/getSystolicBP', function (req, res) {
    sql.connect(dbConfig.dbConnection()).then(() => {
        return sql.query(`select [PERSON_ID], [SUBCATEGORY], [EVENT_CD], [RESULT_VAL],  Category = 'Risk Factors - Non Red Flag' from dbo.Events where category = 'Event' and [SUBCATEGORY] = 'Vitals' and [EVENT_CD] = '102225120' and RESULT_VAL >= 160`);
    }).then(result => {
        console.log(" query result  ", result);
        res.json({ data: result.recordset })
    }).catch(err => {
